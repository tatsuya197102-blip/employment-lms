// seed_quiz_m25_m29_v1.cjs
// M25〜M29 のクイズ75問を Firestore の quizBank に投入する一回きりのスクリプト
//
// 使い方(このファイルと同じ場所に quiz_m25_m29.json とサービスアカウント鍵を置く):
//   node seed_quiz_m25_m29_v1.cjs  <サービスアカウント鍵のファイル名>
//
// 実行後、鍵ファイルとこのスクリプトは削除してください。
...
if (!keyFile) {
  ...
  console.error('例: node seed_quiz_m25_m29_v1.cjs employment-lms-key.json')
  process.exit(1)
}
...
const jsonPath = path.resolve(__dirname, 'quiz_m25_m29.json')
if (!fs.existsSync(jsonPath)) {
  console.error('エラー: quiz_m25_m29.json が見つかりません → ' + jsonPath)
  process.exit(1)
}
...
async function main() {
  ...
  console.log('')
  console.log('完了しました。鍵ファイルとこのスクリプトを削除してください。')
  process.exit(0)
}
