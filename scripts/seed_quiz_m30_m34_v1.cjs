// seed_quiz_m30_m34_v1.cjs
// M30〜M34 のクイズ75問を Firestore の quizBank に投入する一回きりのスクリプト
//
// 使い方(このファイルと同じ場所に quiz_m30_m34.json とサービスアカウント鍵を置く):
//   node seed_quiz_m30_m34_v1.cjs  <サービスアカウント鍵のファイル名>
//
...
if (!keyFile) {
  ...
  console.error('例: node seed_quiz_m30_m34_v1.cjs employment-lms-key.json')
  process.exit(1)
}
...
const jsonPath = path.resolve(__dirname, 'quiz_m30_m34.json')
if (!fs.existsSync(jsonPath)) {
  console.error('エラー: quiz_m30_m34.json が見つかりません → ' + jsonPath)
  process.exit(1)
}
